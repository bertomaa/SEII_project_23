...
describe("Test the playlists api", () => {
    ...
    beforeAll((done)=>{
        ...
        try {
            agent.post("/users/register").send({
                name: "test-user",
    ...
    test("404 on get playlists without existing user without cookies", async (done) => {
        const response = await agent.get('/users/not-a-user/playlists');
        expect(response.status).toBe(404);
        done();
    });

    test("401 on get playlists with existing user without cookies", async (done) => {
        const response = await agent.get('/users/test-user/playlists');
        expect(response.status).toBe(401);
        done();
    });

    test("401 on create playlists without permissions without cookies", async (done) => {
        const response = await agent.put('/users/test-user/playlists').send({"playlist": "test-playlist"});
        expect(response.status).toBe(401);
        done();
    });

    test("401 on edit playlist name without permission without cookies", async (done) => {
        const response = await agent.patch('/users/test-user/playlists/test-playlist').send({"newName": "new-name"});
        expect(response.status).toBe(401);
        done();
    });

    test("200 on successful login", async(done) => {
        const response = await agent.post("/users/login").send({ username: "test-user", password: "test-user-password" });
    ...
    test("401 on create playlists without permissions", async (done) => {
        const response = await agent.put('/users/second-test-user/playlists').send({"playlist": "test-playlist"});
        expect(response.status).toBe(401);
        done();
    });
    ...
    test("200 on add movie to playlist", async (done) => {
        const response = await agent.put('/users/test-user/playlists/test-playlist').send({"movieId": "tt0000009"});
        expect(response.status).toBe(200);
        done();
    });

    test("400 on add movie to playlist without correct data", async (done) => {
        const response = await agent.put('/users/test-user/playlists/test-playlist').send({});
        expect(response.status).toBe(400);
        done();
    });

    test("401 on add movie to playlist without permission", async (done) => {
        const response = await agent.put('/users/second-test-user/playlists/test-playlist').send({"movieId": "tt0000009"});
        expect(response.status).toBe(401);
        done();
    });

    test("404 on add movie to non existing playlist", async (done) => {
        const response = await agent.put('/users/test-user/playlists/not-a-playlist').send({"movieId": "tt0000009"});
        expect(response.status).toBe(404);
        done();
    });
    ...
    test("404 on edit non-existing playlist name", async (done) => {
        const response = await agent.patch('/users/test-user/playlists/not-a-playlist').send({"newName": "new-name"});
        expect(response.status).toBe(404);
        done();
    });
    ...
    test("401 on edit playlist name without permission", async (done) => {
        const response = await agent.patch('/users/second-test-user/playlists/not-a-playlist').send({"newName": "new-name"});
        expect(response.status).toBe(401);
        done();
    });

    test("200 on delete movie from playlist", async (done) => {
        const response1 = await agent.put('/users/test-user/playlists/new-name').send({"movieId": "tt0000009"});
        const response2 = await agent.delete('/users/test-user/playlists/new-name').send({"movieId": "tt0000009"});
        expect(response2.status).toBe(200);
        done();
    });

    test("404 on delete movie from non existing playlist", async (done) => {
        const response = await agent.delete('/users/test-user/playlists/not-a-playlist').send({"movieId": "tt0000009"});
        expect(response.status).toBe(404);
        done();
    });

    test("404 on delete non existing movie from playlist", async (done) => {
        const response = await agent.delete('/users/test-user/playlists/test-playlist').send({"movieId": "not a movieId"});
        expect(response.status).toBe(404);
        done();
    });

    test("400 on delete movie from playlist without correct data", async (done) => {
        const response = await agent.delete('/users/test-user/playlists/new-name').send({});
        expect(response.status).toBe(400);
        done();
    });

    test("401 on delete movie from playlist without permission", async (done) => {
        const response = await agent.delete('/users/second-test-user/playlists/test-playlist').send({"movieId": "tt0000009"});
        expect(response.status).toBe(401);
        done();
    });

    test("200 on delete playlists with permissions", async (done) => {
        const response = await agent.delete('/users/test-user/playlists').send({"playlist": "new-name"});
        expect(response.status).toBe(200);
        done();
    });

    test("401 on delete playlists without permissions", async (done) => {
        const response = await agent.delete('/users/second-test-user/playlists').send({"playlist": "test-playlist"});
        expect(response.status).toBe(401);
        done();
    });

    test("404 on delete non existing playlist", async (done) => {
        const response = await agent.delete('/users/test-user/playlists').send({"playlist": "not-a-playlist"});
        expect(response.status).toBe(404);
        done();
    });
    
    test("200 on successful logout", async(done) => {
        const response = await agent.post("/users/test-user/logout");
        expect(response.status).toBe(200);
        done();
    });
