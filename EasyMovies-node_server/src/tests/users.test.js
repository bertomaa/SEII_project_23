...
describe("Test the users api", () => {
    ...
    test("200 on get user data with existing user", async(done) => {
        const response = await agent.get('/api/v1/users/test-user-username');
        expect(response.status).toBe(200);
        done()
    });
    
    test("404 on get user data with non-existing user", async(done) => {
        const response = await agent.get("/api/v1/users/fake-user");
    ...
    test("400 on login with no password", async(done) => {
        const response1 = await agent.post("/api/v1/users/login").send({ username: "test-user-username"});
        expect(response1.status).toBe(400);
        done();
    });

    test("404 on login with non-existing username", async(done) => {
        const response1 = await agent.post("/api/v1/users/login").send({ username: "fake-user", password: "fake-pass" });
        expect(response1.status).toBe(404);
        done();
    });

    test("401 on login with wrong password", async(done) => {
        const response1 = await agent.post("/api/v1/users/login").send({ username: "test-user-username", password: "test-user-wrong-password" });
        expect(response1.status).toBe(401);
        done();
    });

    test("200 on successful login", async(done) => {
        const response1 = await agent.post("/api/v1/users/login").send({ username: "test-user-username", password: "test-user-password" });
    ...
    test("404 on log out with wrong username", async(done) => {
        const response = await agent.post("/api/v1/users/fake-user/logout");
        expect(response.status).toBe(404);
        done();
    });
    
    test("200 on successful logout", async(done) => {
        const response = await agent.post("/api/v1/users/test-user-username/logout");
        expect(response.status).toBe(200);
        done();
    });
    ...
    test("200 on successful user reviews get", async(done) => {
        const response = await agent.get("/api/v1/users/test-user-username/reviews");
        expect(response.status).toBe(200);
        done();
    });

    test("404 on user reviews get with wrong user", async(done) => {
        const response = await agent.get("/api/v1/users/fake-user/reviews");
        expect(response.status).toBe(404);
        done();
    });
    ...
    test("401 on user deletion with no authorization", async(done) => {
        const response = await agent.delete("/api/v1/users/test-user-username");
        expect(response.status).toBe(401);
        done();
    });

    test("200 on successful user deletion", async(done) => {
        await agent.post("/api/v1/users/login").send({ username: "test-user-username", password: "test-user-password" });
        const response = await agent.delete("/api/v1/users/test-user-username");
        expect(response.status).toBe(200);
        done();
    });
