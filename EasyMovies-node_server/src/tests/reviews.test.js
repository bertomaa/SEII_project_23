...
describe("Test the reviews api", () => {
    ...
    test("401 on put movie review without authorization without cookies", async(done) => {
        const response1 = await agent.put("/movies/tt0000009/reviews").send({ username: "test-user", title: "title", content: "content", rate: 5 });
        expect(response1.status).toBe(401);
        done();
    });

    test("401 on delete movie review without authorization without cookies", async(done) => {
        const response1 = await agent.delete("/movies/tt0000009/reviews").send({ username: "test-user"});
        expect(response1.status).toBe(401);
        done();
    });

    test("401 on patch movie review without authorization", async(done) => {
        const response1 = await agent.patch("/movies/tt0000009/reviews").send({ username: "test-user", title: "new-title", content: "different-content", rate: 4 });
        expect(response1.status).toBe(401);
        done();
    });

    test("200 on successful login", async(done) => {
        const response1 = await agent.post("/users/login").send({ username: "test-user", password: "test-user-password" });
        expect(response1.status).toBe(200);
        done();
    });
    ...
    test("400 on put movie review with invalid data", async(done) => {
        const response1 = await agent.put("/movies/tt0000009/reviews").send({ username: "test-user", title: "title", content: "content", rate: -100 });
        expect(response1.status).toBe(400);
        done();
    });

    test("404 on put movie review on non existing movie", async(done) => {
        const response1 = await agent.put("/movies/not-a-movie/reviews").send({ username: "test-user", title: "title", content: "content", rate: 5 });
        expect(response1.status).toBe(404);
        done();
    });
    ...
    test("401 on patch movie review without authorization", async(done) => {
        const response1 = await agent.patch("/movies/tt0000009/reviews").send({ username: "second-test-user", title: "new-title", content: "different-content", rate: 4 });
        expect(response1.status).toBe(401);
        done();
    });
    ...
    test("404 on patch non-existing movie-review", async(done) => {
        ...
        const response1 = await agent.patch("/movies/tt0000574/reviews").send({ username: "test-user", title: "title", content: "content", rate: 5 });
        expect(response1.status).toBe(404);
        done();
    });
    ...
    test("404 on patch non-existing-movie review", async(done) => {
        ...
        const response1 = await agent.patch("/movies/not-a-movie/reviews").send({ username: "test-user", title: "title", content: "content", rate: 5 });
        expect(response1.status).toBe(404);
        done();
    });
    ...
    test("401 on delete movie review without authorization", async(done) => {
        const response1 = await agent.delete("/movies/tt0000009/reviews").send({ username: "second-test-user"});
        expect(response1.status).toBe(401);
        done();
    });
    ...
    test("404 on delete non-existing-movie review", async(done) => {
        //Movie does not exist
        const response1 = await agent.delete("/movies/not-a-movie/reviews").send({ username: "test-user"});
        expect(response1.status).toBe(404);
        done();
    });
    ...
    test("404 on delete non-existing movie-review", async(done) => {
        //Review does not exist
        const response1 = await agent.delete("/movies/tt0000574/reviews").send({ username: "test-user"});
        expect(response1.status).toBe(404);
        done();
    });
